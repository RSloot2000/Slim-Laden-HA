...
NUMBERS: tuple[PeblarNumberDescription, ...] = (
    PeblarNumberDescription(
        key=SET_DOEL_SOC, setting_key=SET_DOEL_SOC, translation_key=SET_DOEL_SOC,
        name="Doel-SoC",
    ...
    PeblarNumberDescription(
        ...
        translation_key=SET_ACCU_CAPACITEIT_KWH,
    ...
    PeblarNumberDescription(
        key=SET_PV_MARGE_WATT, setting_key=SET_PV_MARGE_WATT,
        translation_key=SET_PV_MARGE_WATT, name="PV-marge",
        native_min_value=0, native_max_value=2000, native_step=10,
        native_unit_of_measurement="W", mode=NumberMode.BOX,
    ),
    PeblarNumberDescription(
        key=SET_MIN_A, setting_key=SET_MIN_A, translation_key=SET_MIN_A,
        name="Laadvermogen min",
        native_min_value=HW_MIN_A, native_max_value=HW_MAX_A, native_step=1,
        native_unit_of_measurement="A", mode=NumberMode.BOX,
    ),
    PeblarNumberDescription(
        key=SET_MAX_A, setting_key=SET_MAX_A, translation_key=SET_MAX_A,
        name="Laadvermogen max",
        native_min_value=HW_MIN_A, native_max_value=HW_MAX_A, native_step=1,
        native_unit_of_measurement="A", mode=NumberMode.BOX,
    ),
    PeblarNumberDescription(
        key=SET_ZON_BENUT_FACTOR, setting_key=SET_ZON_BENUT_FACTOR,
        translation_key=SET_ZON_BENUT_FACTOR,
...
class PeblarNumber(PeblarEntity, NumberEntity):
    ...
    async def async_set_native_value(self, value: float) -> None:
        ...
        # min mag nooit boven max uitkomen: sleep de ander mee.
        if key == SET_MIN_A:
            if int(stored) > int(self.coordinator.get_setting(SET_MAX_A)):
                await self.coordinator.async_set_setting(SET_MAX_A, int(stored))
        elif key == SET_MAX_A:
            if int(stored) < int(self.coordinator.get_setting(SET_MIN_A)):
                await self.coordinator.async_set_setting(SET_MIN_A, int(stored))
